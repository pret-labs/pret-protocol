...
import { eAuroraNetwork, ICommonConfiguration } from '../../helpers/types';
...
task('dev:info', 'Show user and reserve info').setAction(async ({}, localBRE) => {
  ...
  const poolConfig = loadPoolConfig(ConfigNames.Aurora);
  const { ReserveAssets } = poolConfig as ICommonConfiguration;
  const reserveAssets = await getParamPerNetwork(ReserveAssets, eAuroraNetwork.aurora);
  console.log('assets');
