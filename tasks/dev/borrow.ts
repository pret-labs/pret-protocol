...
task('dev:borrow', 'Simple Borrow flow')
  .setAction(async ({}, localBRE) => {
    ...
    // user 1 approve 
    await waitForTx(
      await token1.connect(user1)
        .approve(lendingPool.address, user1DepositAmount)
    );
    ...
    // user 1 deposit
    await waitForTx(
      await lendingPool.connect(user1)
        .deposit(
          token1.address, 
          user1DepositAmount,
          user1Addr,
    ...
    // user 2 approve 
    await waitForTx(
      await token2.connect(user2)
        .approve(lendingPool.address, user2DepositAmount)
    );
    console.log('user2 approved');

    // user 2 deposit 
    await waitForTx(
      await lendingPool.connect(user2)
        .deposit(
          token2.address, 
          user2DepositAmount,
          user2Addr,
